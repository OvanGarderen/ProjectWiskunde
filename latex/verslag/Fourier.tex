...
de looptijd van het algoritme. We hebben hier een stelling uit de complexiteits theorie nodig, voor het bewijs zie Akra-Bazzi \cite{akra-bazzi}
...
\end{stelling}
Dit is een speciaal geval van de stelling die op ons algoritme van toepassing is.
...
Een eigenschap van deze definitie is dat het simpel is om te schrijven is naar het gewone DFT/FFT geval wanneer
we ons richten op een enkele co\"ordinaat. We noteren hier $\DFT_m$ voor de DFT functie die een $m$-dimensionaal ingangssignaal transformeert. Dan kunnen we een recursieve definitie geven:
\begin{definitie}[Recursieve Multidimensionale Discrete Fourier Transformatie]
...
Dit gebeurt door middel van een subroutine die een aantal $\DFT_{m-1}(x[j_m])$ berekent en deze teruggeeft als
een vector $X_{m-1}$ over $j_m$, dan is de $DFT_m$ te implementeren als een subroutine die voor elke 
$k_1\leq n_1,\ldots,k_{m-1}\leq n_{m-1}$ de berekening $\DFT(X_{m-1}[k_1,\ldots,k_{m-1}])$ uitvoert.
Algemener voert de subroutine die $DFT_t$ voor een $m$-dimensionaal signaal geeft de volgende berekening uit.

\begin{algo}[Multidimensionaal DFT Algoritme]
Gegeven is een $m$-dimensionale input $x$ en een huidig level $t$, nu wordt de output $X_t$ gegeven door:
\eq{
  X_{t+1}[k_1,\ldots,k_{t-1},k_{t+1},\ldots,k_m] = \DFT(X_{t}[k_1,\ldots,k_{t-1},k_{t+1},\ldots,k_m]) \\
  \text{voor } k_1\leq n_1, \ldots ,k_{t-1}\leq n_{t-1}, k_{t+1}\leq n_{t+1}, \ldots k_m \leq n_m
}
Waarbij we de randvoorwaarde $X_0 = x$ gebruiken.
\end{algo}
Dit Algoritme wordt aangesproken met input $x$ en level $t=m$ en spreekt zichzelf vervolgens aan met input $x$
en level $t'=t-1$. 
%-----------------------------------------------------------------------------------------------------------------
