...
Wat in de huidige tijd van computers ook een belangrijk punt geworden is, is complexiteit van een algoritme.

De complexiteit vertaalt namelijk direct naar de tijd die nodig is om een resultaat te krijgen. We hebben, om de complexiteit te vinden, de volgende stelling uit de Complexiteitstheorie nodig.\footnote{Dit is een speciaal geval van de stelling, voor de volledige stelleing en het bewijs zie \cite{akra-bazzi}.}
...
\begin{gevolg}
Omdat het algoritme niet in lineaire tijd loopt, wordt het al snel lang wachten bij grote signalen. In meer dimensies is er al helemaal snel een groot praktisch bezwaar. Dit wachten is ook precies de reden dat we driedimensionale signalen niet verder bekeken hebben met de Fouriertransformatie.
\end{gevolg}
...
Het feit dat dit een basis is, kan gevonden worden in \ref{topo}.
\end{definitie}
