...
Lossy compressie wordt veel meer gebruikt bij afbeeldingen van natuurlijk beeld zoals foto's, waar een (klein) verlies van weergave niet erg is terwijl de compressie een substanti\"eel verschil in grootte oplevert. 
Wij zullen naar dit soort compressie kijken. Er zijn een aantal types compressie te onderscheiden, te kennen:
\begin{itemize}
...
De vraag die misschien opkomt is, ``hoe krijgen we nou compressie?". Het antwoord is tweeledig. In theorie is het het makkelijkst om (bijvoorbeeld) de eerste $K$ basisfuncties te gebruiken voor de reconstructie. De verwachte fout die hiermee bereikt kan worden staat beschreven in hoofdstukken \ref{fourierH} en \ref{waveletsH}. Dit is wel onder sterke aannames over het beginsignaal die over het algemeen niet opgaan.
