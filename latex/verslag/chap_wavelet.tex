...
co\"effici\"enten $a_j[n]$ en de \emph{detail}-co\"effici\"enten $d_j[n]$, die
$f$ geven als projectie op  respectievelijk $V_j$ en $W_j$.

Het is echter veel rekenwerk om al deze co\"effici\"enten uit te rekenen. Dit gebeurt immers
door het berekenen van integralen. We beperken ons daarom tot het uit rekenen van de
co\"effici\"enten op het niveau $j$ en proberen vervolgens een recursieve relatie te vinden
om hieruit de approximatie- en detailco\"efficienten van het volgende niveau te vinden.
...
in termen van de coe\"ffici\"enten op de resolutie $2^j$.
\begin{equation}
  \label{approx_rec}
...
\begin{algo}[Fast Wavelet Transform]
  ...
  \end{equation}
\end{algo}
We gaan uit van een eindige filter en beschouwen onze ruimte $V_j$ als functies
op een interval in $\R$, dan versimpelen namelijk de oneindige sommaties in \eqref{approx_rec},
\eqref{detail_rec}. Voor een discrete functie $f\in\R^{2^j}$ kunnen we nu
...
Wanneer we een benadering van $f$ maken op resolutie $2^J$ (door bijvoorbeeld een Fast Wavelet Transform), bekijken we
\[
...
Als de lengte van de filter $h$ gelijk is aan $K$, en de lengte van het originele signaal $a_L$ gelijk is aan $N = 2^{L}$,
...
\] 
We vinden voor $V_j^{(2)}$ de ruimte van stuksgewijs constante functies op vierkanten $[2^{-j}n_1, 2^{-j}(n_1+1)) \times [2^{-j}n_2, 2^{-j}(n_2+1))$. De tweedimensionale schalingsfunctie wordt op die manier
\[
...
\begin{equation}
  \label{2d_ruimte_rec}
  V_{j+1}^{(2)} =  V_j^{(2)} \oplus \boldsymbol{U}_j
\end{equation}
...
tweedimensionale wavelets in de ruimte $V_j^{(2)}$ gegeven door:
\begin{equation}
  \label{psi_k_defs}
...
\]
we zullen bewijzen dat deze functies tezamen met $\phi^{(2)}_{0,n}$een basis vormen voor $L_2(\R)$
\begin{stelling}[{\cite[T7.24]{mallat}}]
  \label{mallatbasis}
  Laat $\phi^{(2)}$ en de $\psi^k$'s zoals hierboven gedefinieerd,
  dan kunnen we hiermee een basis $\boldsymbol{\Psi}_j$ maken voor $\boldsymbol{U}_j$ volgens:
  \[
  \boldsymbol\Psi_j := \{ \psi^1_{j,n}, \psi^2_{j,n}, \psi^3_{j,n}: n \in \Z^2 \},
  \] 
  het samennemen van deze bases gecombineerd met een basis voor $V_0^{(2)}$ geeft bovendien 
  \[
...
\begin{proof}[Bewijs]
  ...
  \]
  en splitsen de termen, die \'e\'en-dimensionale ruimtes zijn, op in de volgende niveau's
  ($V_{j+1} = V_j \oplus W_j$) en vinden dat:
  \begin{equation*}
  \begin{split}
...
\end{proof}
\begin{gevolg}
We kunnen de laatste regels van het bewijs ook gedeeltelijk toepassen om te vinden dat 
\[
\left\{ \phi^{(2)}_{0, n}: n \in \Z^2 \right\} \cup \left\{ \psi^1_{j,n}, \psi^2_{j,n}, \psi^3_{j,n}: n \in \Z^2, j \in \{0, \ldots, L \}  \right\}
\]
een basis voor $V_L^{(2)}$ is.
\end{gevolg}

Bovenstaande basis heeft dus het tensorproduct van twee schalingsfuncties op \'e\'en niveau 
en verder combinaties van schaling en wavelet op alle niveaus. 
...
Nu we een relatie hebben gevonden $(\eqref{2d_ruimte_rec})$ tussen de ruimtes op verschillende
niveau's volgens
\begin{equation}
...
\]
We zullen in het bijzonder kunnen zeggen (maak gebruik van $\eqref{phi_phi_basis}$)
...
\begin{equation}
  ...
  \phi^{(2)}_{j+1,(k_1,k_2)}
\end{equation}
We maken nu gebruik van \cite{weidmann}[\S 3.4] om het inproduct horende bij het tensorproduct
...
\]
Deze stappen kunnen ook met hetzelfde argument toegepast worden op de $\psi^k$'s,
...
\]
Vervolgens richten we ons weer op de \emph{approximatie} en \emph{detail}~co\"effici\"enten door
aan beide zeiden van de vergelijkingen $\eqref{phi_phi_som}$,$\eqref{psi_k_som}$ het inproduct
...
\begin{proof}[Bewijs]
  ...
  \begin{equation*}
    \begin{split}
      ...
      \end{split}
  \end{equation*}
  Wat precies gelijk is aan de convolutie in \eqref{iFWTd_def}. Daarmee geeft de iFWT een inverse voor de FWT.
\end{proof}
...
waarbij we ditmaal het tensorproduct nemen van $m$ factoren $V_{j-1}$ en $W_{j-1}$.
...
Herinner dat we voor de Mallat-decompositie gebruik maakte van de gelijkheid (zie \eqref{2d_ruimte_decomp}):
\[
...
Bedenk dat we in 1 dimensie de decompositie zoals in \eqref{ruimte_splitsing} gebruiken volgens:
\[
...
bij de Tensorbasis hebben we echter $j_1$ en $j_2$ die ongelijk aan elkaar zijn.
...
Met deze theoretische kennis willen we een algoritme bedenken dat een tweedimensionaal signaal schrijft in termen van de Tensorbasis,
...
het Tensorproduct toegepast worden op meer dan $2$ dimensies. We zullen beargumenteren dat de beide
...
\end{voorbeeld}
\begin{gevolg}
Wanneer we de eerste term opvatten als een ruimte van afbeelding op een bepaald moment en
...
Wanneer we niet de hele basis bekijken, maar slechts de eerste $N$ elementen pakken, 
krijgen we een verzameling $\mathcal{B}_N \subset \mathcal{B}$ zodat
\[
f_{\mathcal{B}_N} := \sum_{m = 1}^N \langle f, g_m \rangle g_m.
\]
In het bijzonder zijn we op zoek naar de \emph{fout} $\| f - f_{\mathcal{B}_N} \|$:
\[
...
\begin{proof}
  ...
  \]
  en de notie dat $p$ constant is voor een keuze van de wavelet. De fout is  $2^{-Mp/2} \simeq 2^{-Mp}$. Omschrijven geeft dat dit overeenkomt met een fout van $N^{-p/n}$.
\end{proof}
...
\end{stelling}
\begin{proof}
  We kunnen met het lemma \ref{en_daarom_jongens_en_meisjes} om de fout af te schatten:
  \[
  ...
  \]
  en dit is precies wat we willen bewijzen.
\end{proof}
...
Niet \emph{helemaal}, omdat per constructie van onze wavelets de basis compact is zijn alleen 
lokaal wavelets nodig zijn om een discontinuiteit te reconstrueren;
we `zoomen we in' op de functie. Lokaal is de functie $f$ meestal wel glad zodat de benadering daar goed is.

Een gevolg van de verschillen in lokale gladheid is dat we wavelets beter moeten selecteren: meer wavelets rond een discontinuiteit en minder op de gladde delen. Hiervoor gebruiken we een zogenaamde \emph{adaptieve} deelverzameling die selecteert op de grootte van de co\"efficienten.

\subsection{Vergelijking met Fouriertransformatie}
Wanneer we het convergentiegedrag van de wavelet co\"efficienten vergelijken met de daling 
van de co\"effici\"enten van de Fouriertransformatie dan zien we dat de convergentie van zijn
co\"efficienten loopt volgens $N^{-(k-1)}$ voor $f\in C^k$. De convergentie hangt dus sterk samen
met de gladheid van de functie, terwijl dit bij de wavelets afhangt van de gekozen wavelet.

Zodra de functie een discontinuiteit vertoont treden er problemen op met de Fouriertransformatie,
het reconstrueren van de discontinuiteit wordt bemoeilijkt en de reconstructie heeft enkel nog zachte
randen. In zo'n situatie verwachten we dan ook dat de wavelettransformatie betere resultaten vertoont.
