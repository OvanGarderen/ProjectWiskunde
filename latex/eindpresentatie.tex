...
\frame{
  \frametitle{Recap}
  \begin{itemize}
  \item Project bij Rob Stevensson
  \item JPEG-2000
  \item Successor van JPEG $\implies$ Fourier
  \item Discreet signaal (in 1, 2 of 3 dimensies)
  \end{itemize}
}
...
\frame{
  ...
  \begin{columns}
    \begin{column}{0.6\linewidth}
      \begin{itemize}
      \item Mallat, approximatie algoritme van functie
      \item Op zoek naar $a_\lambda = \langle f, \phi_\lambda \rangle$
      \item Recursieve relatie: $\phi(x) = \sum_k h_k \phi(2x-k)$
      \end{itemize}
\bigskip
      Mallat:
      \begin{eqnarray*}
...
\begin{columns}
  \begin{column}{0.6\linewidth}
    ...
    \item \'e\'en approximatie kwadrant en drie detail kwadranten
...
\frame{
  ...
  \item Opeenvolgende frames op vatten als 3de dimensie
  \end{itemize}
}
