...
Schrijf dus $Q_{z} : L_2([0,1]) \rightarrow V_z$ Voor de (lineaire) projectie-afbeelding die een
functie $f$ afbeeldt op zijn projectie in de ruimte $V_z$.
Dan geldt dat het inproduct tussen de wavelet functie en $f$ ook in andere vorm is te schrijven:
\eq{
  \langle f , \phi_\abso{\lambda} \rangle = \langle (I - Q_{\abso{\lambda}-1})(f) , \phi_\abso{\lambda} \rangle 
}
...
op niveau $M$, zodat het aantal wavelets $N \in \O((M+1)\cdot2^M )$

Daarmee is de orde van de fout in termen van het aantal gebruikte wavelets van orde $\O(lel)
