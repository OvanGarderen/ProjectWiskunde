...
\begin{columns}
  ...
  \begin{column}{.5\textwidth}
    ...
    \item $\Rightarrow$ Fourier transformatie
    \item Voorbeeldje
    }
  \end{column}
\end{columns}
...
\frame{
  \frametitle{Werkt FFT wel echt als DFT? Ja!}
DEZE SLIDE FAALT
  De co\"efficienten FFT wordt gegeven door de volgende vergelijking:
  \[
  ...
  \]
  Werkt door symmetrie in de basis-functies.
}

\frame{
  Voorbeeldvraag:
  \lijstje{
  \item- waarom zou je uberhaupt FFT doen
  \item- significantie coeffs (toepassing babyvoorbeeld)
  \item- FFT op plaatje (2d fft), hoe werkt het? -> FFT op rijen van plaatje, dan FFT op kolommen van je FFT (ofzo?)
  }
}
