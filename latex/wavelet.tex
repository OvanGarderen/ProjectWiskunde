...
TODO: kijk even wat je hiervan vind, ik heb alle changes rood gemaakt.
\bigskip

\noindent
We weten dat $V_j^2$ bevat is in $V_{j-1}^2$. 
We bekijken het orthogonale complement ${\color{red} U_j} \perp V_j^2$:
\[
	V_j^2 \oplus U_j = V_{j-1}^2.
\]
Om nu een orthogonale waveletbasis voor ${\color{red} U_j}$ en (dus) $L^2(\R^2)$ te vinden, gaan we als volgt te werk.
...
Dan is $\{ \psi^1_{j,n}, \psi^2_{j,n}, \psi^3_{j,n}: n \in \Z^2 \}$ een basis voor ${\color{red} U_j}$ 
en $\{ \psi^1_{j,n}, \psi^2_{j,n}, \psi^3_{j,n}: n \in \Z^2, j \in \Z \}$ een basis voor $L^2(\R^2)$.
\end{stelling}
\begin{proof}[Bewijs]
We weten
\[
	V_{j-1}^2 = V_j^2 \oplus {\color{red} U_j} \implies V_{j-1} \otimes V_{j-1} = ( V_j \otimes V_j ) \oplus {\color{red} U_j}.
\]
Vul nu $V_{j-1} = V_j \oplus W_j$ in om te vinden
\[
	( V_j \oplus W_j ) \otimes (V_j \oplus W_j ) = (V_j \otimes V_j) \oplus {\color{red} U_j} %hier stond dus:(W_j \otimes W_j) 
\]
\[
	\implies (V_j \otimes V_j) \oplus (V_j \otimes W_j) \oplus (W_j \otimes V_j) \oplus (W_j \otimes W_j) = (V_j \otimes V_j) \oplus {\color{red} U_j}
\]
\[
	\implies (V_j \otimes W_j) \oplus (W_j \otimes V_j) \oplus (W_j \otimes W_j) = {\color{red} U_j}.
\]
Nu is het duidelijk dat $\{ \psi^1_{j,n}, \psi^2_{j,n}, \psi^3_{j,n}: n \in \Z^2 \}$ een basis is voor ${\color{red} U_j}$. Omdat nu
\[
